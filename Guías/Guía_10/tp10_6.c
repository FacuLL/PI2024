...
// OJO, NO SIEMPRE SE EJECUTA CON l->tail. Cuando se elimina uno hay que seguir con el mismo.
...
void deleteDupl (TList l) {
    ...
    else deleteDupl(l->tail);
}

// ME HABIA OLVIDADO DE HACER EL l->tail = res. OJO CUANDO RES ES NULL

TList deleteDupl2 (TList l) {
    if (l == NULL) return NULL;
    TList res = deleteDupl2(l->tail);
    if (res != NULL && res->elem == l->elem) {
        free(l);
        return res;
    }
    l->tail = res;
