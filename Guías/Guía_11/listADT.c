...
static tList deleteRec(tList list, elemType elem, cmp cmpFun, int * res) {
    ...
    list->tail = deleteRec(list->tail, elem, cmpFun, res);
    return list;
}

int delete(listADT list, elemType elem) {
    int res = 0;
    deleteRec(list->list, elem, list->cmpFun, &res);
    list->elemCount-=res;
    return res;
}
